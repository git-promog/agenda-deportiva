...
export default function ShareButton({ titulo, slug, url, className = '', variant = 'full' }: ShareButtonProps) {
  ...
  const compartirWhatsApp = () => {
    ...
    trackEvent('share', { method: 'whatsapp', content_type: 'article', item_id: slug || titulo });
  ...
  const compartirTwitter = () => {
    ...
    trackEvent('share', { method: 'twitter', content_type: 'article', item_id: slug || titulo });
    window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(texto)}&url=${encodeURIComponent(shareUrl)}`, '_blank');
    setAbierto(false);
  };

  const compartirFacebook = () => {
    trackEvent('share', { method: 'facebook', content_type: 'article', item_id: slug || titulo });
    window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`, '_blank');
    setAbierto(false);
  };

  const compartirEmail = () => {
    trackEvent('share', { method: 'email', content_type: 'article', item_id: slug || titulo });
  ...
  const copiarEnlace = async () => {
    trackEvent('share', { method: 'copy_link', content_type: 'article', item_id: slug || titulo });
    try {
