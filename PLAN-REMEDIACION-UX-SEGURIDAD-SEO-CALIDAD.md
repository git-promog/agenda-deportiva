...
1. **Reactivar Deployment Protection** (paso 7 de la sección Vercel): volver a `Settings → Deployment Protection → Vercel Authentication` y activar (o limitar a usuarios del equipo). Registrar aquí fecha/hora. Obligatorio antes de cerrar el plan o desplegar a producción. La validación externa (Rich Results + Search Console) ya terminó, por lo que **es el siguiente paso inmediato**.
